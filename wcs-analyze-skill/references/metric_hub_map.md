...
| Low **slot travel range** (down-slot, per dancer) | Movement-Concepts/Ideas; *(Partnering-Concepts)* | dancer doesn't traverse the slot — staying planted instead of travelling from one end to the other | travel the slot, down the slot, send her down, post-and-travel, cover the slot, go somewhere |
| Low **couple travel around room** (spotlight only) | Movement-Concepts/Ideas | in a spotlight, the couple stays parked instead of using the floor — IGNORE for contained/prelim clips (lower is expected, not a gap) | use the floor, travel the room, spotlight, cover ground, journey, stage |
| Low **stretch/compression after post** | Partnering-Mechanics/Concepts | the post is set but little body travel comes out of it — weak elastic payoff | stretch out of the anchor, slingshot, leverage, send, expand off the post, time under tension |
